...
{
    if (c < 'A')return c;   /* Numbers, remain the same. */

    if (c >= 'a')
    {
        return c - 0x20;    /* Becomes uppercase. */
    }
    else
    {
        return c;           /* Uppercase, left unchanged */
    }
}
        ...
        if (res == 0)
        {
            ...
            if (res == 0)   									/* Successfully opened the directory */
            {
                ...
                if (fn == 0)									/* Label copy */
                {
                    ...
                    dstpathname[dstpathlen] = psrc[0];  /* Record the volume label */
                    dstpathname[dstpathlen + 1] = 0;    /* End character */
                }
                else strcat((char *)dstpathname, (const char *)fn); /* Add file name */

                fcpymsg(fn, 0, 0X04);   /* Update the folder name */
                res = f_mkdir((const TCHAR *)dstpathname);/* If the folder already exists, it is not created. Create a new folder if it does not exist. */

                if (res == FR_EXIST)res = 0;

                while (res == 0)
                {
                    res = f_readdir(srcdir, finfo);         /* Reads a file in the directory */

                    if (res != FR_OK || finfo->fname[0] == 0)break;

                    if (finfo->fname[0] == '.')continue;    /* Ignore the parent directory */

                    fn = (uint8_t *)finfo->fname;           /* Gets the file name */
                    dstpathlen = strlen((const char *)dstpathname); /* Gets the length of the current destination path */
                    srcpathlen = strlen((const char *)srcpathname); /* Gets the source path length */
                    ...
                    {
                        strcat((char *)dstpathname, (const char *)"/"); /* Destination path with slash */
                        strcat((char *)dstpathname, (const char *)fn);  /* Destination path plus file name */
                        strcat((char *)srcpathname, (const char *)fn);  /* Source path plus file name */
                        fcpymsg(fn, 0, 0X01);       /* Updates the file name */
                        res = exfuns_file_copy(fcpymsg, srcpathname, dstpathname, *totsize, *cpdsize, fwmode);  /* copy file  */
                        *cpdsize += finfo->fsize;   /* Increases the file size by */
                    }

                    srcpathname[srcpathlen] = 0;    /* Adds the end character */
                    dstpathname[dstpathlen] = 0;    /* Adds the end character */
                }
            }
