    ...
    if (htim->Instance == TIM5)
    {
        if ((g_timxchy_cap_sta & 0X80) == 0)                	/* No capture yet. */
        {
            if (g_timxchy_cap_sta & 0X40)                   	/* A falling edge is captured. */
            {
                ...
                __HAL_TIM_SET_CAPTUREPOLARITY(&htim5, TIM_CHANNEL_1, TIM_INPUTCHANNELPOLARITY_RISING);/* Configure TIM3 channel 1 rising edge capture. */
            }
            else /* Not yet started, first capture rising edge */
            {
                ...
                __HAL_TIM_DISABLE(&htim5);          				/* Turn off timer 3 */
                __HAL_TIM_SET_COUNTER(&htim5, 0);   				/* Timer 3 The counter is cleared */
                TIM_RESET_CAPTUREPOLARITY(&htim5, TIM_CHANNEL_1); 	/* Be sure to clear the original Settings first */
                __HAL_TIM_SET_CAPTUREPOLARITY(&htim5, TIM_CHANNEL_1, TIM_INPUTCHANNELPOLARITY_FALLING);/* Timer 3 Channel 1 is set to falling edge capture */
                __HAL_TIM_ENABLE(&htim5);           				/* Enable timer 3 */
            }
        }
    }
}
