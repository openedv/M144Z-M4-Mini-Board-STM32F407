...
/******************************************************************************************/
/* The following code was added by the ALIENTEK team to use TJPGD */
...
/* he allocation method depends on whether malloc is used */
#if JPEG_USE_MALLOC == 1    /* use malloc */

FIL *f_jpeg;                /* JPEG file pointer */
JDEC *jpeg_dev;             /* Object structure pointer to be decoded */
uint8_t  *jpg_buffer;       /* Define the size of the jpeg decoding workspace (minimum 3,092 bytes), which must be 4-byte aligned for the decompression buffer */

/**
 * @brief  requests memory for large arrays/structs
 * @param  None
...
static uint8_t jpeg_mallocall(void)
{
    ...
    if (f_jpeg == NULL)return PIC_MEM_ERR;      /* Failed memory request */

    jpeg_dev = (JDEC *)piclib_mem_malloc(sizeof(JDEC));

    if (jpeg_dev == NULL)return PIC_MEM_ERR;    /* Failed memory request */

    jpg_buffer = (uint8_t *)piclib_mem_malloc(JPEG_WBUF_SIZE);

    if (jpg_buffer == NULL)return PIC_MEM_ERR;  /* Failed memory request */
...
{
    piclib_mem_free(f_jpeg);        /* Free the memory allocated by f_jpeg */
    piclib_mem_free(jpeg_dev);      /* Free the memory reserved by jpeg_dev */
    piclib_mem_free(jpg_buffer);    /* Release the memory requested by the jpg buffer */
}
...
FIL  tf_jpeg;
JDEC tjpeg_dev;
FIL  *f_jpeg = &tf_jpeg;                /* JPEG file pointer */
JDEC *jpeg_dev = &tjpeg_dev;            /* Object structure pointer to be decoded */

/* Define the size of the jpeg decoding workspace (minimum 3,092 bytes), which must be 4-byte aligned for the decompression buffer */
__ALIGNED(4) uint8_t jpg_buffer[JPEG_WBUF_SIZE];

#endif

/**
 * @brief    jpeg data entry callback function
 * @param    jd  : A struct that stores information about the object to be decoded
 * @param    buf : input data buffer (NULL: execution address offset)
 * @param    num : The amount of data/address offset to read from the input data stream
 * @retval   read bytes/address offset
 */
static size_t jpeg_in_func(JDEC *jd, uint8_t *buf, size_t num)
{
    uint16_t  rb;   /* The number of bytes read */
    FIL *dev = (FIL *)jd->device;           /* The information of the file to be decoded is defined using the FIL structure type in FATFS */

    if (buf)        /* Read data valid, start reading data */
    {
        f_read(dev, buf, num, (UINT *)&rb); /* Call FATFS 'f_read function to read out the data from the jpeg file */
        return rb;  /* Returns the number of bytes read */
    }
    else
    {
        return (f_lseek(dev, f_tell(dev) + num) == FR_OK) ? num : 0;    /* Relocating a data point is equivalent to deleting the previous n bytes of data */
    }
}

/**
 * @brief  decode and display images with padding (fast)
 * @param  jd     : A struct that stores information about the object to be decoded
 * @param  rgbbuf : A pointer to RGB bitmap data waiting to be output
 * @param  rect   : Parameters of the rectangle image to wait for output
 * @retval operation result
 * @arg    0, output success
 * @arg    other, output failed/end output
 */
static int jpeg_out_func_fill(JDEC *jd, void *rgbbuf, JRECT *rect)
{
    uint16_t *pencolor = (uint16_t *)rgbbuf;
    uint16_t width = rect->right - rect->left + 1;  /* Width of fill */
    uint16_t height = rect->bottom - rect->top + 1; /* height of fill */
    pic_phy.fillcolor(rect->left + picinfo.S_XOFF, rect->top + picinfo.S_YOFF, width, height, pencolor);    /* Color fill */
    return 0;   /* 0 is returned to allow decoding to continue */
}

/**
 * @brief  Decodes and displays the image by drawing dots (slow)
 * @param  jd     : A struct that stores information about the object to be decoded
 * @param  rgbbuf : A pointer to RGB bitmap data waiting to be output
 * @param  rect   : Parameters of the rectangle image to wait for output
 * @retval operation result
...
static int jpeg_out_func_point(JDEC *jd, void *rgbbuf, JRECT *rect)
{
    ...
    uint16_t width = rect->right - rect->left + 1;  /* Image width */
    uint16_t height = rect->bottom - rect->top + 1; /* Image height */

    for (i = 0; i < height; i++)    /* y coordinate */
    {
        realy = (picinfo.Div_Fac * (rect->top + i)) >> 13;  /* Actual Y coordinates */
        ...
        for (j = 0; j < width; j++)   /* x-coordinate */
        {
            realx = (picinfo.Div_Fac * (rect->left + j)) >> 13; /* Actual X coordinates */

            if (!piclib_is_element_ok(realx, realy, 1))
            {
                pencolor++;
                continue;
            }

            pic_phy.draw_point(realx + picinfo.S_XOFF, realy + picinfo.S_YOFF, *pencolor);  /* display picture */
            pencolor++;
        }
    }
...
/**
 * @brief  Gets the width and height of a JPEG/JPG image
 * @param  filename : The filename containing the path (.jpeg/.jpg)
...
uint8_t jpg_get_size(const char *filename, uint32_t *width, uint32_t *height)
{
    ...
    if (res == 0)
    {
        /* Returns the beginning of the JPEG/JPG image */
        res = f_open(f_jpeg, (const TCHAR *)filename, FA_READ); /* open file */

        if (res == FR_OK)   /* Opened file successfully */
        {
            res = jd_prepare(jpeg_dev, jpeg_in_func, jpg_buffer, JPEG_WBUF_SIZE, f_jpeg);   /* To prepare for decoding, call the jd_prepare function of the TjpgDec module */

            if (res == JDR_OK)  /* Ready to decode successfully, that is, the width and height of the image are obtained. Returns the correct width and height */
            {
        ...
        f_close(f_jpeg);    /* The decoding is successful, and 0 is returned */
    }

#if JPEG_USE_MALLOC == 1    /* use malloc */
    jpeg_freeall();         /* free the memory */
#endif
    return res;
}
...
{
    uint8_t res = 0;    /* returned value */
    uint8_t scale;      /* Image output scale 0,1/2,1/4,1/8 */
    ...
    if (res == 0)
    {
        /* Get the beginning information of the JPEG/JPG image */
        res = f_open(f_jpeg, (const TCHAR *)filename, FA_READ); /* open-file */

        if (res == FR_OK)       /* Opened file successfully */
        {
            res = jd_prepare(jpeg_dev, jpeg_in_func, jpg_buffer, JPEG_WBUF_SIZE, f_jpeg);   /* To prepare for decoding, call the jd_prepare function of the TjpgDec module */
            outfun = jpeg_out_func_point;   /* By default, it is displayed by drawing dots */

            if (res == JDR_OK)   /* Ready to decode successfully */
            {
                for (scale = 0; scale < 4; scale++)   /* Determine the scale factor of the output image */
                {
                    if ((jpeg_dev->width >> scale) <= picinfo.S_Width && (jpeg_dev->height >> scale) <= picinfo.S_Height)   /* In the target area */
                    {
                        if (((jpeg_dev->width >> scale) != picinfo.S_Width) && ((jpeg_dev->height >> scale) != picinfo.S_Height && scale))
                        {
                            scale = 0;  /* If you can't edge, you don't scale */
                        }
                        else
                        {
                            outfun = jpeg_out_func_fill;    /* Within the display size, it can be displayed by filling */
                        }
                ...
                if (scale == 4)scale = 0;   /* error */

                if (fast == 0)   /* No need for fast decoding */
                {
                    outfun = jpeg_out_func_point;   /* By default, it is displayed by drawing dots */
                }

                picinfo.ImgHeight = jpeg_dev->height >> scale;  /* The resized image size */
                picinfo.ImgWidth = jpeg_dev->width >> scale;    /* The resized image size */
                piclib_ai_draw_init();  /* Initialize the smart drawing */

                /* To perform the decoding, call the jd decomp function of the TjpgDec module */
